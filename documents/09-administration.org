...
Org lets you easily convert list items to headings and back again. The =C-c C-*= keyboard shortcut (~org-ctrl-c-star~) converts a paragraph to a heading. If the line contains a checkbox, it becomes a to-do item. Using =C-c C-c -= (~org-ctrl-c-minus~) converts a paragraph or a heading to a list item.

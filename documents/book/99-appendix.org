...
Access the ~customise-variable~ function with the =C-c w v= shortcut.
...
The Denote-Org package by Prot enhances functionality when working with Org files. The two bound commands provide the ability to link to a heading inside an Org note or convert a heading to a new node. This package also adds Denote-specific functionalities to Org mode's dynamic blocks, available through the ~org-dynamic-block-insert-dblock~ command (=C-c C-x x=).

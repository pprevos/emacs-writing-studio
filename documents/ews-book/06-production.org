...
In the throws of the writing process, it is not uncommon to change your mind on how a text should flow or even totally change its structure. To ensure you retain valuable information, you need to understand how Emacs manages different versions of a buffer or a file.

Writing may seem like a solitary pursuit, but collaborating with fellow authors is often needed. In such scenarios, version control also plays a pivotal role. While Emacs may not boast the flashy cloud collaboration systems found in office software, its integrated version control system allows for seamless collaboration on a project, ensuring no contributions are lost.

Emacs offers a range of methods to control versions of your files. At the lowest level, we have the version in the current buffer and the one saved to disk. The undo system meticulously tracks all changes within the current buffer at a more granular level. Another method involves the built-in backup system, which saves older versions of files, preserving your manuscript's evolution. For more advanced needs, Emacs also provides version control methods that allow you to formally register files in a version control system.
...
** Automated Backups
Rewriting a file automatically destroys all record of its previous contents, which sometimes means loosing many hours of writing within a split second. To prevent such disasters, Emacs can keep a backup of every file.

Emacs backups a file the first time you ad content to a file. The first backup of any new file is empty. No matter how many times you subsequently save the file, its backup remains unchanged until you kill the buffer and visit the file again. So backup files contain the versions just before starting a new writing session. This backup will be the same as the current file, until the next save.  If you save the buffer with a prefix argument (=C-u C-x C-s=), the saved version will become a backup file. The backup version 
...
EWS is also configured to keep the last three versions of the file. Emacs appends version numbers to the end of the filename for a backup: =~1~=, =~2= and so on.

Use the directory editor (~dired~) to view the available backup files. Use the =C-x d= shortcut and enter the location of the backup folder. 

The way Emacs manages backups can be a tad confusing, so lets summarise (figure [[#fig:backup]]). When you create a new file and start editing, a Emacs creates a backup, which at this stage is an empty file. When you obtain a file from elsewhere, the first backup is the state in which you first opened in Emacs. While you are editing and saving intermediate versions, no new backups are created, unless you save the file with the universal argument. Emacs automatically creates a new backup, when you open it again and start editing. You can also revert to the previous saved version and you can view versions of the current buffer with the Undo Tree package. These mechanisms provide fine-tuned version control that minimises the risk of loosing information to close to zero.
...
#+begin_src dot :file images/backups.png
  digraph backups {
    ...
    file2 [label = "File\nrev. 1"]
    file3 [label = "File\nrev. 2"]
    backup1 [label = "Backup\nno. 1"]
    backup2 [label = "Backup\nno. 2"]
    backup3 [label = "Backup\nno. 3"]

    file1 -> file1 [label="undo-tree"]
    file1 -> file2 
    file2 -> file1 [label="revert-buffer"]
    file2 -> file2 [label="undo-tree"]  
    file2 -> file3 
    file3 -> file2 [label="revert-buffer"]
    file3 -> file3 [label="undo-tree"]  
...
1. Register the file under version control =C-x v v= (~vc-next-action~). DWIM
2. Next logical action: commit changes
   - Add comment on the changes
   - When no changes are saved, then minibuffer "Fileset is up-to-date"
   - Enter =C-c C-c=

The ~vc-diff~ command shows a popup buffer that visualises the differences between the current version and the lates commit. Any text that is removed is shown on a red background and new text is marked green. 

Produce a list of changes with =C-x v L= or ~vc-print-root-log~ (quit with =q=). You can also view the evolution of the file with the =d= key on the relevant version. Selecting more than one commit with the =m= and arrow keys and then =d= shows the differences between the oldest and latest version.

The ~vc-dir~ command (=C-x v d=) lists all files in the current directory and their status. The first line shows the backend for this version controlled directory, which in our case is 

The stach is a place to safely store a version 

Emacs ignores creating backups for files that are managed by version control to prevent duplication.

#+begin_src dot :file images/git.png
  digraph git {
    graph [dpi=300]
    edge [dir=none]
    ...
    subgraph cluster1 {
      ...
      style=filled
      color=lightgrey
      version3 [label="Version 1"]
      version2 [label="Version 2"]
      version1 [label="Version 3"]
      version1 -> version2 -> version3
    }

    file -> version1 [constraint=false]
  }
#+end_src
#+caption: Local version Git diagram.
#+name: fig:git
#+RESULTS:
[[file:images/git.png]]
...
The EWS configuration disables lock files, so if you need this functionality you will need to customise the ~create-lockfiles~ variable.
